...
$ackn = "";
$data = [];
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ackno'])) {
    $ackn = $_POST['ackno'];
    $res = $conn->query("SELECT * FROM monthlyreport WHERE ackno='$ackn'");
    if ($res->num_rows > 0) {
        $row = $res->fetch_assoc();

    } else {
        echo "<div class='alert alert-danger'>No record found for ACK No: $ackn</div>";
        exit;
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $preparedat = $_POST['preparedat'];
    $update = $conn->prepare("UPDATE monthlyreport SET 
        customerName=?, customerAddress=?, panNo=?, gstNo=?, state=?, stateCode=?, 
        invoiceNo=?, invoiceDate=?, particular=?, sacCode=?, amount=?, cgst=?, sgst=?, 
        total=?, totalText=?, preparedby=?, preparedat=?, applygst=?
        WHERE ackno=?");
    $update->bind_param(
        "ssssssssssddddsssss",
        $customerName,
...
<body>
    <table class='bordered' style='padding:12px !important;'>
        ...
        <tr>
            ...
            <td colspan='3' rowspan='3' style='padding-bottom:5px;padding-left:0px'>: " . $customerAddress . "</td>
...
<body>
    ...
    <section id="content">
        ...
        <div class="container form-container">
            <form method="POST">
                <input type="hidden" name="ackno" value="<?php echo htmlspecialchars($row['ackno']); ?>">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col">
                        ...
                        <input type="text" name="customerAddress" class="form-control"
                            value="<?php echo $row['customerAddress']; ?>">
                    </div>
                </div>
                ...
                <div class="row mb-2">
                    <div class="col">
                        ...
                        <input type="text" name="gstNo" id="gstNo" class="form-control"
                            value="<?php echo $row['gstNo']; ?>">
                    </div>
                    <div class="col">
                        <label>PAN No (auto from GST)</label>
                        <input type="text" name="panNo" id="panNo" class="form-control"
                            value="<?php echo $row['panNo']; ?>" readonly>
                    </div>
                </div>
                ...
                <div class="row mb-2">
                    <div class="col">
                        ...
                        <select name="state" id="state" class="form-select">
                            ...
                            foreach ($states as $code => $name) {
                                $selected = ($row['state'] == $name) ? "selected" : "";
                    ...
                    <div class="col">
                        ...
                        <input type="text" name="stateCode" id="stateCode" class="form-control"
                            value="<?php echo $row['stateCode']; ?>" readonly>
                    </div>
                </div>
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="particular" class="form-control" value="<?php echo $row['particular']; ?>">
                </div>

                <div class="row mb-2">
                    <div class="col">
                        <label>SAC Code</label>
                        <input type="text" name="sacCode" class="form-control" value="<?php echo $row['sacCode']; ?>">
                    </div>
                    <div class="col">
                        <label>Amount</label>
                        <input type="number" step="0.01" name="amount" id="amount" class="form-control"
                            value="<?php echo $row['amount']; ?>" oninput="calcTaxes()">
                    </div>
                    <div class="col-12 mb-2">
                        <label>
                            <input type="checkbox" id="applygst" name="applygst" value="1" <?php echo $row['applygst'] == 1 ? 'checked' : ''; ?> onclick="calcTaxes()">
                            Apply GST
                        </label>
                    </div>
                </div>
                ...
                <div class="row mb-2">
                    <div class="col">
                        ...
                        <input type="hidden" name="preparedby" class="form-control"
                            value="<?php echo $row['preparedby']; ?>" readonly>
                    </div>
                    <div class="col">

                        <input type="hidden" name="preparedat" class="form-control"
                            value="<?php echo $row['preparedat']; ?>" readonly>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById("gstNo").addEventListener("input", function () {
            let gst = this.value;
            if (gst.length >= 12) {
                document.getElementById("panNo").value = gst.substring(2, 12);
            }
        });
        ...
        // Run once on page load to adjust based on DB value
        window.onload = calcTaxes;
    </script>
    <script>
