...
if (isset($_POST['confirm'])) {
    if (isset($_POST['selectedVehicles'])) {
        $selectedVehicles = $_POST['selectedVehicles']; // Array of selected vehicle numbers

        foreach ($selectedVehicles as $vehicleNumber) {
            // Fetch the comma-separated docket numbers for the selected vehicle number
            $query = "SELECT docket_no FROM vehicle_details WHERE vehicle_number = '$vehicleNumber'and Ini_Loc='$location' AND status = 'New'";
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    $docketNos = $row['docket_no']; // Comma-separated docket numbers
                    $docketNoArray = explode(',', $docketNos); // Split into an array

                    foreach ($docketNoArray as $docketNo) {
                        $docketNo = trim($docketNo); // Trim any whitespace

                        // Update the status of the docket number in the invoices table
                        $updateQuery = "UPDATE invoices SET transit_status = 'Moved' WHERE billid = '$docketNo'";
                        if ($conn->query($updateQuery) !== TRUE) {
                            $allSuccess = false;
                            echo "<div class='alert alert-danger mt-3'>Error updating status for docket number $docketNo: " . $conn->error . "</div>";
                        }
                    }
                }

                // Update the status of the vehicle in the vehicle_details table
                $updateVehicleQuery = "UPDATE vehicle_details SET status = 'Moved' WHERE vehicle_number = '$vehicleNumber' and Ini_Loc='$location' and status='New'";
                if ($conn->query($updateVehicleQuery) !== TRUE) {
                    $allSuccess = false;
                    echo "<div class='alert alert-danger mt-3'>Error updating status for vehicle number $vehicleNumber: " . $conn->error . "</div>";
                }

                // Show a single success alert if all operations were successful
                if ($allSuccess) {
                    echo "<div class='alert alert-success mt-3'>All statuses updated successfully!</div>";
                }
            } else {
                echo "<div class='alert alert-warning mt-3'>No new docket numbers found for vehicle number $vehicleNumber.</div>";
            }

        }
    } else {
        echo "No vehicles selected for status update.";
    }
}
        ...
        <ul class="side-menu">
            ...
            <li>
                ...
                <ul class="side-dropdown show">
                    ...
                    <li><a href="Admin_VehicleOUT.php" style="color: #2f3192">Vehicle OUT</a></li>
        ...
        <div class="container form-container">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <input type="text" name="searchQuery" class="form-control"
                            placeholder="Search by Docket Number or Vehicle Number"
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <form method="post" id="confirmationForm">
                    <input type="submit" class="btn btn-success mb-3" name="confirm" value="Confirm" id="confirmButton">
            ...
            <div class="responsive-table mt-4">
                <table class="table table-bordered ">
                    ...
                    <tbody>
                        <?php
                        $sta = 'New'; // Status 'New' for vehicle details
                        if (strtolower($position2) === 'admin') {
                            $query = "SELECT * FROM vehicle_details WHERE status = '$sta'"; // Base query for vehicle details
                        } else {
                            $query = "SELECT * FROM vehicle_details WHERE status = '$sta' and Ini_Loc='$location'"; // Base query for vehicle details
                        }
                        ...
                        if (!empty($_GET['searchQuery'])) {
                            ...
                            if (strpos($search, 'SV') === 0) { // If it starts with 'SV', assume it's a docket number
                                $query .= " AND docket_no LIKE '%$search%'";
                            } else { // Otherwise, it's a vehicle number
                                $query .= " AND vehicle_number LIKE '%$search%'";
                            }
                        }
                        ...
                        $query .= " ORDER BY created_at DESC"; // Optional: Adjust sorting as needed
                        $result = $conn->query($query);

                        if ($result->num_rows > 0) {
                            $serialNo = 1;
                            $vehicleData = []; // Store vehicle details by vehicle number
                        
                            while ($row = $result->fetch_assoc()) {
                                $vehicleNumber = $row['vehicle_number'];
                                $price = $row['price'];
                                $docketNumber = $row['docket_no']; // Fetch docket number for each vehicle
                        
                                // Store the docket number for each vehicle number
                                if (!isset($vehicleData[$vehicleNumber])) {
                                    $vehicleData[$vehicleNumber] = [];
                                }
                                $vehicleData[$vehicleNumber][] = $docketNumber;
                            }

                            // Loop through each vehicle number and display the docket numbers
                            foreach ($vehicleData as $vehicleNumber => $docketNumbers) {
                                $docketList = implode(", ", $docketNumbers); // Combine docket numbers in a string
                        
                                // Display the row
                                echo "<tr>
                        <td><input type='checkbox' name='selectedVehicles[]' value='$vehicleNumber' class='rowCheckbox'></td>
                        <td>{$serialNo}</td>
                        <td>{$vehicleNumber}</td>
                        <td>";
                                ...
                                foreach ($docketNumbers as $docketNumber) {
                                    ...
                                    $separatedDockets = explode(',', $docketNumber);

                                    // Map each docket number to an anchor tag
                                    $anchorLinks = array_map(function ($sepDocket) {
                                        $sepDocket = trim($sepDocket); // Remove any extra spaces
                                        return "<a href='#' data-bs-toggle='modal' data-bs-target='#viewModal{$sepDocket}'>{$sepDocket}</a>";
                                    }, $separatedDockets);

                                    // Join the anchor links with commas
                                    echo implode(', ', $anchorLinks);
                                }

                                echo "</td><td>{$price}</td></tr>";

                                // Modal for each docket number
                                foreach ($docketNumbers as $docketNumber) {
                                    // Split the comma-separated string into an array
                                    $separatedDockets = explode(',', $docketNumber);

                                    // Iterate over the array of individual docket numbers
                                    foreach ($separatedDockets as $sepDocket) {
                                        $sepDocket = trim($sepDocket); // Remove any extra spaces
                                        echo "
            <div class='modal fade' id='viewModal{$sepDocket}' tabindex='-1' aria-labelledby='viewModalLabel{$sepDocket}' aria-hidden='true'>
                <div class='modal-dialog modal-lg'>
                    <div class='modal-content'>
                        <div class='modal-header'>
                            <h5 class='modal-title' id='viewModalLabel{$sepDocket}'>View PDF for Docket Number: {$sepDocket}</h5>
                            <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
                        </div>
                        <div class='modal-body'>
                            <iframe src='./Bills/{$sepDocket}.pdf' style='width: 100%; height: 70vh; border: 1px solid #ccc;'></iframe>
                        </div>
