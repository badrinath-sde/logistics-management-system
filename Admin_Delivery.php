    ...
    if ($imageFile['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $validFileTypes)) {
            ...
            if (move_uploaded_file($imageFile['tmp_name'], $targetFile)) {
                ...
                $query = "UPDATE invoices SET proof_image = '$fileName',Transit_status = '$sta1',deliveredtime = NOW() WHERE billid = '$billid'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_reason'])) {
    $billid = $_POST['reason_billid'];
    $reason = $conn->real_escape_string($_POST['delivery_reason']);

    $query = "UPDATE invoices SET delivery_status = '$reason' WHERE billid = '$billid'";
    if ($conn->query($query) === TRUE) {
        echo "<script>alert('Reason updated successfully for $billid');</script>";
    } else {
        echo "Error updating reason: " . $conn->error;
    }
}
...
<body>
    ...
    <section id="sidebar">
        ...
        <ul class="side-menu">
            ...
            <li>
                <a href="#" class="active"><i class="bx bxs-inbox icon"></i> Delivery <i
                        class="bx bx-chevron-right icon-right"></i></a>
                <ul class="side-dropdown show">
                    <li><a href="Admin_Delivery.php" style="color: #2f3192">InDelivery</a></li>
                ...
                <table class="table table-bordered full-table">
                    ...
                    <tbody>
                        ...
                        if (strtolower($position2) === 'admin') {
                            ...
                            $query = "SELECT * FROM invoices WHERE consignee_district = '$location' AND transit_status = '$sta'"; // Base query
                        }
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            <td><button class='btn btn-info btn-sm view-btn' data-bs-toggle='modal' data-bs-target='#viewModal{$row['billid']}'>View</button></td>
                                ...
                                if (empty($row['delivery_status'])) {
                                    echo "<button class='btn btn-danger btn-sm reason-btn' data-bs-toggle='modal' data-bs-target='#reasonModal{$row['billid']}'>Give Reason</button>";
                                } else {
                                    echo htmlspecialchars($row['delivery_status']);
                                }
                                echo "</td>
                            <td><button class='btn btn-warning btn-sm edit-btn' data-bs-toggle='modal' data-bs-target='#editModal{$row['billid']}'>Upload</button></td>
                        ...
                        <div class='modal fade' id='viewModal{$row['billid']}' tabindex='-1' aria-labelledby='viewModalLabel{$row['billid']}' aria-hidden='true'>
                            <div class='modal-dialog modal-lg'>
                                ...
                                // Modal for file upload (Edit Modal)
                                echo "
                        <div class='modal fade' id='editModal{$row['billid']}' tabindex='-1' aria-labelledby='editModalLabel{$row['billid']}' aria-hidden='true'>
                            <div class='modal-dialog'>
                                <div class='modal-content'>
                                    <div class='modal-header'>
                                        <h5 class='modal-title' id='editModalLabel{$row['billid']}'>Upload Image for Docket {$row['billid']}</h5>
                                        <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
                                    </div>
                                    <div class='modal-body'>
                                ...
                                echo "  <div class='modal fade' id='reasonModal{$row['billid']}' tabindex='-1' aria-labelledby='reasonModalLabel{$row['billid']}' aria-hidden='true'>
    <div class='modal-dialog'>
        <div class='modal-content'>
            <form action='' method='POST'>
                <div class='modal-header'>
                    <h5 class='modal-title' id='reasonModalLabel{$row['billid']}'>Provide Reason for Docket {$row['billid']}</h5>
                    <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
                </div>
                <div class='modal-body'>
                    <input type='hidden' name='reason_billid' value='{$row['billid']}'>
                    <label for='reason'>Select Reason:</label>
                    <select name='delivery_reason' class='form-control' required>
                       <option value='Customer Seal Issue'>Customer Seal Issue</option>
                        <option value='Local Truck Delay'>Local Truck Delay</option>
                        <option value='Route Vehicle Delay'>Route Vehicle Delay</option>
                        <option value='Customer Not Available'>Customer Not Available</option>
                        <option value='Topay Not Ready'>Topay Not Ready</option>
                        <option value='Change of Address'>Change of Address</option>
                        <option value='Customer Rejected'>Customer Rejected</option>
                        <option value='Delivered but Not POD updated'>Delivered but Not POD updated</option>
                        <option value='ODA'>ODA</option>
                        <option value='Part Shipment'>Part Shipment</option>
    ...
    <td>{$row['consignee_district']}</td>
    <td>
        <button class='btn btn-info btn-sm view-btn' data-bs-toggle='modal' data-bs-target='#viewModal{$row['billid']}'>View</button>
    </td>
    <td>";
                                if (empty($row['delivery_status'])) {
                                    echo "<button class='btn btn-danger btn-sm reason-btn' data-bs-toggle='modal' data-bs-target='#reasonModal{$row['billid']}'>Give Reason</button>";
                                ...
                                echo "</td>
    <td>
        <button class='btn btn-warning btn-sm edit-btn' data-bs-toggle='modal' data-bs-target='#editModal{$row['billid']}'>Upload</button>
    </td>
</tr>";
                                // Modal for PDF view
                                echo "
