            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    try {
                        ...
                        $mail->addAttachment($pdfname, 'Invoice.pdf'); // 'Invoice.pdf' is the name shown in the email
                        ...
                        $allSuccess = false; // If any email fails, set the flag to false


                    }
                }
            } else {
                $allSuccess = false; // If any record is missing, set the flag to false
            }
        }

        // Display the alert after completing all operations
        if ($allSuccess) {
...
<body>
            ...
            <div class="responsive-table mt-4">
                <table class="table table-bordered full-table">
                    ...
                    <tbody>
                        ...
                        $tod2 = date(format: 'Y-m-d');
                        $sta = 'Not_initiated';
                        if (strtolower($position2) === 'admin') {
                            $query = "SELECT * FROM invoices WHERE transit_status = '$sta' and billdate='$tod2'"; // Base query
                        
                        } else {
                            $query = "SELECT * FROM invoices WHERE created_by = '$location' AND transit_status = '$sta' and billdate='$tod2'"; // Base query
                        }
    ...
    <td><button type='button' class='btn btn-warning btn-sm' onclick='handleButtonClick(\"" . $row['billid'] . "\")'>Edit</button></td>
     <td>
    <button type='button' class='btn btn-danger btn-sm' onclick='cancelInvoice(\"" . $row['billid'] . "\")'>Cancel</button>
</td>                           
</tr>";
